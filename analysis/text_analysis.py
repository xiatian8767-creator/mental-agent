...
class TextAnalysis:
    ...
    def run(self) -> None:
        ...
        if text is None:
            return
        if not isinstance(data["analysis"]["text"], dict):
            raise ValueError("analysis.text 必须是对象。")

        prompts = self.read_prompt()
        analysis_prompt = Template(prompts["analysis_prompt"]).substitute(text=text)
        first_result = self.call_api(analysis_prompt)
        description, reasoning = self._read_description_reasoning(first_result)

        data["analysis"]["text"]["description"] = description
        data["analysis"]["text"]["reasoning"] = reasoning
        ...
        second_result = self.call_api(result_prompt)
        result = self._read_result(second_result)

        data["analysis"]["text"]["result"] = result
        self.write_analysis(data)
